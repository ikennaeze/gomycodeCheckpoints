...
function GC_MemberAdder(props) {
  ...
  function handleCheckboxChange(friend, checked){
    if (checked) {
      setGcProps((prevProps) => ({
        ...prevProps,
        gcMembers: [...prevProps.gcMembers, friend],
      }));
    } else {
      setGcProps((prevProps) => ({
        ...prevProps,
        gcMembers: prevProps.gcMembers.filter((member) => member !== friend),
      }));
    }
  }
  ...
  return (
    <>
    <div className={`${props.isGcMemberAdderOpen ? "block" : "hidden"} flex justify-center`}>
      ...
      <span onClick={() => {props.closeGcMemberAdder(false)}} className="material-symbols-outlined text-[20pt] text-[#98ebfa] cursor-pointer duration-300 hover:translate-x-[-6px] absolute top-9 left-6">arrow_back</span>
