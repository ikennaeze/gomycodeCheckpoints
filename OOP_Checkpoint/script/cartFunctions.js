    ...
    addItem = (event) => {
        ...
        if(Number(totalNumberOfItems.innerHTML) == 0){
            ...
            for(let i = 0; i < cartItems.children.length; i++){
                console.log(cartItems.children[i].getAttribute('id'))

                let existingItemQuantity = cartItems.children[i].children[2].children[1]
                let existingItemID = cartItems.children[i].getAttribute('id')
                let itemInCart = item.id == existingItemID
                let subtotal = document.getElementById('subtotal')
                let taxes = document.getElementById('taxes')
                let totalPrice = document.getElementById('total-price')
        ...
        `
                ...
                }else{//otherwise increase the quantity of the existing cart item
                    existingItemQuantity.innerHTML = Number(existingItemQuantity.innerHTML) + 1
                    ...
                    for(let i = 0; i < cartItemAmounts.children.length; i++){
                        ...
                        //if the cart item amount name is the same as the item name, remove that shit too
                        if(itemAmountID == item.id){
                            itemAmount.innerHTML = '$' + (Number(itemAmount.innerHTML.replace(/[$]/g, '')) + item.price)
                            itemAmountQuantity.innerHTML = Number(itemAmountQuantity.innerHTML) + 1
                            //increase the price
                            subtotal.innerHTML = '$' + (Number(subtotal.innerHTML.replace(/[$]/g, '')) + item.price)
                            taxes.innerHTML = '$' + (Number(subtotal.innerHTML.replace(/[$]/g, '')) * 0.13)
                            totalPrice.innerHTML = '$' + (Number(subtotal.innerHTML.replace(/[$]/g, '')) + Number(taxes.innerHTML.replace(/[$]/g, '')))
                        }
                    }
